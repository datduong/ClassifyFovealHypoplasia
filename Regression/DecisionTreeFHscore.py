...
MODEL = {
  'DecisionTreeFH':'eye_position_od,eye_position_os,machine_type_z,machine_type_hb,age_taken,logMAR,spherical_equivalent,nystagmus,dAniridia,dCHS,dErdheim Chester disease,dFH isolated,dHPS,dNanophthalmos,dOA,dOCA,dWaardenburg 2A,gGPR143,gHPS1,gMITF,gOCA2,gPAX6,gPRSS56,gSLC45A2,gTYR,gunknown'.split(','),

  'DecisionTreeFHMeta2+logMAR':'machine_type_z,machine_type_hb,age_taken,spherical_equivalent,nystagmus,logMAR'.split(','),
}

for modelname in sorted(list ( MODEL.keys() )) : 
  ...
  for fold in [0,1,2,3,4]: 
    ...
    for max_depth in np.arange(4,max): 
      ...
      clf = tree.DecisionTreeClassifier(max_depth=max_depth, random_state=fold*10) # print(clf.tree_.max_depth)
      clf = clf.fit(X, Y)
      # tree.plot_tree(clf) 
      ...
      if record_best == 1: 
        ...
        Ytest_pred_score = np.matmul (clf.predict_proba(Xtest), np.reshape([1,2,3,4],(4,1))).flatten()
  ...
  for fold in [0,1,2,3,4]: 
    ...
    # df = pd.read_csv(dfcsv,na_values="-") # ! if we want to test on one of the training folds
    # df = df.dropna()
    ...
    prob_df = pd.DataFrame( PROBS, columns=[str(i) for i in np.arange(4)]) # @PROBS is shape=(1, obs, labelsize)
