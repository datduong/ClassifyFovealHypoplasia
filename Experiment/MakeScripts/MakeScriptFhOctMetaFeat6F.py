...
python evaluate.py --image-csv $imagecsv --kernel-type $kernel_type --model-dir $modeldir --log-dir $logdir --image-size $imagesize --enet-type tf_efficientnet_b4_ns --oof-dir $oofdir --batch-size 32 --num-workers 4 --fold 'FOLD' --out-dim 4 --CUDA_VISIBLE_DEVICES 0 --dropout $dropout --do_test --n-test $ntest --meta-features 'METAFEAT' --n-meta-dim $metadim  # ! actual test set
...
# METAFEAT = 'eye_position_od,eye_position_os,machine_type_z,machine_type_hb,age_taken,logMAR,spherical_equivalent,nystagmus,dAniridia,dCHS,dErdheim Chester disease,dFH isolated,dHPS,dNanophthalmos,dOA,dOCA,dWaardenburg 2A,gGPR143,gHPS1,gMITF,gOCA2,gPAX6,gPRSS56,gSLC45A2,gTYR,gunknown' # ! ok to use logMAR here

METAFEAT = 'machine_type_z,machine_type_hb,age_taken,spherical_equivalent,nystagmus'
      ...
      for schedulerscaler in [10]:
        for learn_rate in [0.00001]:  # 0.00001,0.00003  # we used this too, 0.0001
          for dropout in [0.2]:
            ...
            # os.system('sbatch --time=20:00:00 --mem=64g --cpus-per-task=20 ' + scriptname )
            counter = counter + 1 
