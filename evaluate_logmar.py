...
def val_epoch(model, loader, our_label_index, diagnosis2idx, n_test=1, get_output=True, fold=None, args=None):
    ...
    with torch.no_grad():
        for (data, target, target_soft, path, data_resize) in tqdm(loader): # @path is needed to select our labels
            ...
            # ! ! UNABLE TO DO ATTRIBUTION, BECAUSE CAPTUM WORKS ON DISCRETE LABEL?
            # if args.attribution_keyword is not None: 
    ...
    return LOGITS, val_loss, corr, r2score
...
def main():
    ...
    for fold in [int(i) for i in args.fold.split(',')]:
        ...
        dataset_valid = DatasetFromCsv(df_valid, 'valid', args.meta_features, transform=transforms_val, transform_resize=transforms_resize, soft_label=args.soft_label, linear_loss=args.linear_loss)
        ...
        this_LOGITS, val_loss, corr, r2score = val_epoch(model, valid_loader, our_label_index, diagnosis2idx, n_test=args.n_test, get_output=True, fold=fold, args=args)
        LOGITS.append(this_LOGITS)

        # print 
        content = time.ctime() + ' ' + f'Fold {fold}, valid loss: {(val_loss):.5f}, corr: {(corr):.4f}, r2score {(r2score):.6f}'
        print(content)
        with open(os.path.join(args.log_dir, f'log_{args.kernel_type}_eval.txt'), 'a') as appender:
            appender.write(content + '\n')

        # ! merge data frame
        print ('PROB output size {}'.format(LOGITS[0].shape))
        prob_df = pd.DataFrame( LOGITS[0], columns=[str(i) for i in np.arange(args.out_dim)]) # @PROBS is shape=(1, obs, labelsize)
