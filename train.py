...
def parse_args():
    ...
    parser.add_argument('--new-label', type=str, default=None) # override label col with some other column
...
def val_epoch(model, loader, n_test=1, is_ext=None, get_output=False, criterion=None, args=None):
    ...
    PROBS = torch.cat(PROBS).numpy() # ! 2D array
    ...
    if get_output:
        ...
        return val_loss, acc, bal_acc, bal_acc_ourdata
...
def run(fold, df, meta_features, n_meta_features, transforms_train, transforms_val, criterion):
    ...
    train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=args.batch_size, sampler=RandomSampler(dataset_train), num_workers=args.num_workers, drop_last=True) # ! avoid single batch size, which BatchNorm can't handle 
    ...
    if args.loaded_model is not None: 
        ...
        if args.use_meta or (args.out_dim!=4): # ! the pretrained models has 4 labels. 
...
def main():
    ...
    if (args.weighted_loss is not None) and (args.label_upweigh is not None): # ! bring @criterion so we can use weighted loss
        args.weighted_loss = [float(i) for i in args.weighted_loss.split('-') ] # ! easier to retain name with - than it is ; 
